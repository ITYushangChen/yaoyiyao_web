...
class Room {
  ...
  snapshot(forRole = 'screen') {
    ...
    if (forRole === 'screen') {
      ...
      return {
        ...base,
        participants: showLiveRanking ? participants : [],
    ...
    for (const p of this.players.values()) {
      if (p.nickname === name) {
        ...
        if (this.phase === PHASE.STARTING && this.startIntro) {
          ws.send({ type: 'start_intro', phase: PHASE.STARTING, ...this.startIntro });
        }
        if (p.shakeCount > 0) {
          ws.send({
            type: 'shaken',
  ...
  autoReveal() {
    if (this.phase !== PHASE.OPEN) return { ok: false, message: '当前不在摇动阶段' };
    this.clearRoundTimer();
  ...
  /** 全员 progress 节流：避免 200 人狂点时 O(n²) 广播打满局域网 */
