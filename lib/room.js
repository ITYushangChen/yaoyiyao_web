...
/** 入场 lobby 全员广播合并窗口（避免 join 时 O(n²)） */
const LOBBY_THROTTLE_MS = 300;
/** 等待/倒计时阶段大屏刷新（含全员名单） */
const SCREEN_WAITING_THROTTLE_MS = 500;
/** 开局后大屏实时榜刷新 */
const SCREEN_LIVE_THROTTLE_MS = 120;
...
class Room {
  constructor(roomId, options = {}) {
    ...
    this._lobbyThrottle = null;
    this._pendingLobby = false;
  ...
  clearScreenThrottle() {
    if (this._screenThrottle) {
      clearTimeout(this._screenThrottle);
      this._screenThrottle = null;
    }
    this._pendingScreenPush = false;
  }
  ...
  clearPlayerThrottle() {
    if (this._playerThrottle) {
      clearTimeout(this._playerThrottle);
      this._playerThrottle = null;
    }
    this._pendingPlayerProgress = false;
  }
  ...
  clearLobbyThrottle() {
    if (this._lobbyThrottle) {
      clearTimeout(this._lobbyThrottle);
      this._lobbyThrottle = null;
    }
    this._pendingLobby = false;
  }
  ...
  clearAllTimers() {
    ...
    this.clearRankTimer();
    this.clearScreenThrottle();
    this.clearPlayerThrottle();
    this.clearLobbyThrottle();
  }
  ...
  snapshot(forRole = 'screen') {
    ...
    if (forRole === 'screen') {
      ...
      const showRoster =
        this.phase === PHASE.WAITING || this.phase === PHASE.STARTING;
      const liveTop = this.config.liveTopCount || 20;
      return {
        ...base,
        // 实时榜只下发 Top N；全员名单仅等待/倒计时阶段下发
        participants: showLiveRanking ? participants.slice(0, liveTop) : [],
        joinedPreview: showRoster
  ...
  addPlayer(ws, nickname) {
    ...
    const maxPlayers = this.config.maxPlayers || 220;
    if (this.players.size >= maxPlayers) {
      ws.send({
        type: 'error',
        message: `本场已满（${maxPlayers} 人），请稍后再试`,
      });
      return null;
    }
  ...
  removePlayer(ws) {
    ...
    this.broadcastScreenThrottled();
    if (seatFreed) this.broadcastLobbyThrottled();
    this.markDirty();
  }
  ...
  broadcastScreenThrottled() {
    ...
    const delay =
      this.phase === PHASE.WAITING || this.phase === PHASE.STARTING
        ? SCREEN_WAITING_THROTTLE_MS
        : SCREEN_LIVE_THROTTLE_MS;
  ...
  /** 入场人数变化：合并推送给全员，避免每人 join 广播一次 */
  broadcastLobbyThrottled() {
    this._pendingLobby = true;
    if (this._lobbyThrottle) return;
    this._lobbyThrottle = setTimeout(() => {
      this._lobbyThrottle = null;
      if (!this._pendingLobby) return;
      this._pendingLobby = false;
      this.broadcastPlayers({
        type: 'lobby',
        participantCount: this.players.size,
        ...
        phase: this.phase,
        serverNow: Date.now(),
        maxPlayers: this.config.maxPlayers || 220,
      });
    }, LOBBY_THROTTLE_MS);
  }
