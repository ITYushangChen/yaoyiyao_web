...
class Room {
  ...
  addPlayer(ws, nickname, meta = {}) {
    ...
    const openId = meta.openId ? String(meta.openId).slice(0, 64) : '';
    const fromWechat = !!meta.fromWechat;

    // Reconnect: same WeChat openId or same nickname reuses existing seat
    for (const p of this.players.values()) {
      const sameWx = openId && p.openId && p.openId === openId;
      const sameName = !openId && p.nickname === name;
      if (sameWx || sameName) {
        if (p.ws && p.ws.isOpen() && p.ws !== ws) {
          ws.send({ type: 'error', message: sameWx ? '该微信已在其他设备入场' : '昵称已被使用，请换一个' });
          return null;
        }
        p.ws = ws;
        if (name) p.nickname = name;
        if (openId) p.openId = openId;
        if (fromWechat) p.fromWechat = true;
        ws.roomId = this.id;
        ws.role = 'player';
        ws.playerId = p.id;
        ws.send({
